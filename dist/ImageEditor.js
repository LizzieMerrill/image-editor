...
class ImageEditor {
    static read(filePath) {
        const input = fs_1.default.readFileSync(filePath, "utf-8").split(/\s+/);
        if (input[0] !== "P3") {
    ...
    static applyEmboss(sourceImage) {
    ...
    static applyInvert(sourceImage) {
        const outputImage = new Image(sourceImage.width, sourceImage.height);
    ...
    static applyGrayscale(sourceImage) {
        const outputImage = new Image(sourceImage.width, sourceImage.height);
        for (let y = 0; y < sourceImage.height; y++) {
            for (let x = 0; x < sourceImage.width; x++) {
                const color = sourceImage.get(x, y);
    ...
    static applyMotionBlur(sourceImage) {
        const outputImage = new Image(sourceImage.width, sourceImage.height);
        for (let y = 0; y < sourceImage.height; y++) {
            for (let x = 0; x < sourceImage.width; x++) {
    ...
    static run(sourceImagePath, keyImagePath, filterType) {
